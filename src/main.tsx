...
function LoginPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try { await signIn(email, password); navigate((location.state as { from?: string } | null)?.from ?? '/dashboard', { replace: true }) }
...
function DashboardPage() {
  ...
  return <><PageHeader eyebrow="WORKSPACE OVERVIEW" title="Good morning." action={<Link className="button button-primary" to="/calls/new">+ Analyse a call</Link>} />{summary.isError ? <ErrorState message={summary.error.message} /> : <><div className="stats-grid"><StatCard label="Total calls" value={summary.data?.totalCalls ?? '—'} hint="All time" /><StatCard label="Completed" value={summary.data?.completedCalls ?? '—'} hint="Processed successfully" tone="positive" /><StatCard label="Failed" value={summary.data?.failedCalls ?? '—'} hint="Need attention" tone="negative" /><StatCard label="Average score" value={summary.data?.averageScore == null ? '—' : `${Math.round(summary.data.averageScore)}/100`} hint="Across completed calls" tone="accent" /></div><section className="content-card"><div className="section-heading"><div><p className="eyebrow">RECENT ACTIVITY</p><h2>Latest calls</h2></div><Link className="text-link" to="/calls">View all →</Link></div>{calls.isError ? <ErrorState message={calls.error.message} /> : <CallTable calls={calls.data?.calls ?? []} compact={true} />}</section></>}</>
}

function statusLabel(status: CallStatus) { return ({ uploaded: 'Uploaded', queued: 'Queued', transcribing: 'Transcribing', transcribed: 'Transcribed', analyzing: 'Analysing', completed: 'Completed', failed: 'Failed' } satisfies Record<CallStatus, string>)[status] }
...
function CallsPage() {
  ...
  return <><PageHeader eyebrow="CALL LIBRARY" title="Calls" action={<Link className="button button-primary" to="/calls/new">+ Upload call</Link>} /><section className="content-card"><div className="section-heading"><div><h2>All recordings</h2><p className="muted">Review processing status and open detailed analysis.</p></div>{calls.data && <span className="result-count">{calls.data.total} total</span>}</div>{calls.isLoading ? <div className="loading-line" /> : calls.isError ? <ErrorState message={calls.error.message} /> : <><CallTable calls={calls.data?.calls ?? []} />{(calls.data?.totalPages ?? 1) > 1 && <div className="pagination"><button disabled={page === 1} onClick={() => setPage(value => value - 1)}>←</button><span>Page {page} of {calls.data?.totalPages}</span><button disabled={page === calls.data?.totalPages} onClick={() => setPage(value => value + 1)}>→</button></div>}</>}</section></>
}

function UploadPage() {
  const navigate = useNavigate(); const queryClient = useQueryClient(); const [file, setFile] = useState<File | null>(null); const [dragging, setDragging] = useState(false)
